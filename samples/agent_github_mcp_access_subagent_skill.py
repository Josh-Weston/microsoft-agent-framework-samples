...
from agent_framework import MCPStreamableHTTPTool, FileAgentSkillsProvider
...
async def main():
    ...
    async with http_client, mcp_tool:
        ...
        github_skill_agent = Agent(
            client=chat_client,
            name="github_skill",
            description=(
                "Call this skill to interact with GitHub. You can search for repositories, "
                "read file contents, and check issues. Pass the user's exact request as the input."
            ),
            instructions="You are a GitHub specialist. Use your MCP tools to fulfill the user's request.",
            tools=[mcp_tool],
...
"""
            ...
            tools=[github_skill_agent.as_tool()],
            default_options={

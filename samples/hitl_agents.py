"""
In this example, the workflow is re-run when there is a request/response. I don't know how this would scale in
a production workflow if only a part of the workflow needs to be re-run based on a human response...
"""
...
class TurnManager(Executor):
    ...
    async def on_agent_response(
        ...
        # Update the state so the agent can the history of guesses
        guesses = ctx.get_state("guesses", default=[])
        guesses.append(last_guess)
        ctx.set_state("guesses", guesses)
    ...
    async def on_human_feedback(
        ...
        reply = human_response.strip().lower()
        ...
        guess = ctx.get_state("guesses", default=[])
        guesses = ", ".join(str(g) for g in guess)
        feedback_text = (
            f"Feedback: {reply}. Your previous guesses were: {guesses}. "
...
async def main() -> None:
    ...
    guessing_agent = chat_client.as_agent(
        name="GuessingAgent",
        instructions=(
        ...
        default_options={
            ...
            # switching to a larger model fixed validation issues
    ...
    workflow = (
        ...
        # Ask agent to make/adjust a guess
        .add_edge(turn_manager, guessing_agent)
        # Agent's response comes back to coordinator
